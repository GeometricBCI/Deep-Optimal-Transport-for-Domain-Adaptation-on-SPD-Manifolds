...
class Zero_Centered_FilterBank:
    def __init__(self, fs, pass_width=4, f_width=4):
        self.fs = fs
        self.f_trans = 2
        self.f_pass = np.arange(4, 40, pass_width)
    ...
    def filter_data(self,eeg_data,window_details={}):
        ...
        new_eeg_data = np.zeros((n_trials, n_channels, n_samples)) 

        for i in range(n_trials):
          for j in range(n_channels):
              new_eeg_data[i, j] = eeg_data[i, j] - np.mean(eeg_data[i, j])
        ...
        for i, fb in self.filter_coeff.items():
            ...
            b = fb.get('b')
            a = fb.get('a')
            eeg_data_filtered = np.asarray([signal.lfilter(b,a, new_eeg_data[j,:,:]) for j in range(n_trials)])
